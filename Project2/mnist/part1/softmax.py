...
def compute_probabilities(X, theta, temp_parameter):
    ...
    x = np.transpose(X)
    dotProducts = np.dot(theta,x)/temp_parameter
    maxOfColumns = dotProducts.max(axis = 0)/temp_parameter    
...
#    for j in range(k):
#        c_matrix[j, :] = (np.dot(theta[j, :], np.transpose(X)))/temp_parameter
#    c = np.max(c_matrix, axis=0)
#    e = np.exp(c_matrix-c)
#    s = np.sum(e, axis=0)
#    H = (1/s)*e
#    return H
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    regulizingCost *= lambda_factor / 2.0 
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    nonRegularizedGradient *= -1.0/(np.dot(numExamples, temp_parameter))
    return theta - alpha * (nonRegularizedGradient + lambda_factor * theta)
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    """
    #YOUR CODE HERE
    predict_Y9 = get_classification(X, theta, temp_parameter)
    predict_Y3 = np.remainder(predict_Y9,3)
    return 1 - np.mean(predict_Y3 == Y)
    #raise NotImplementedError
#pragma: coderesponse end
...
def compute_test_error(X, Y, theta, temp_parameter):
    NewLabels = get_classification(X, theta, temp_parameter)
    return 1 - np.mean(NewLabels == Y)
