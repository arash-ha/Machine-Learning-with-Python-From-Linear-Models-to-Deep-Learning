...
def pegasos_single_step_update(
    ...
    """
    # Your code here
    if not label*(np.dot(current_theta, feature_vector)+current_theta_0) > 1:
       return ((1-eta*L)*current_theta + eta*label*feature_vector, current_theta_0 + label)
...
def pegasos(feature_matrix, labels, T, L):
    ...
    current_theta_0 = 0
    p = (current_theta, current_theta_0)
    count = 0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            count = count + 1
            eta = 1/(np.sqrt(count))
            feature_vector = np.squeeze(np.asarray(feature_matrix[i,:]))
            current_theta = np.squeeze(np.asarray(p[0]))
            current_theta_0 = np.squeeze(np.asarray(p[1]))
            label = labels[i]
            p = pegasos_single_step_update(feature_vector, label, L, eta, current_theta, current_theta_0)
...
def classifier_accuracy(
    ...
    #theta = np.squeeze(np.asarray(THE[0]))
    #theta_0 = np.squeeze(np.asarray(THE[1]))
